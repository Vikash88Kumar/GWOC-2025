...
export const BackgroundLines = ({
  ...
  return (
    <div
      className={cn(
        "relative h-[20rem] md:h-screen w-full overflow-hidden bg-transparent dark:bg-black",
        className
      )}
    >
...
const SVG = ({
  ...
  const paths = [
    ...
    // 👉 keep ALL remaining paths exactly as in your original code
  ];
  ...
  return (
    ...
    >
      {Array.from({ length: STROKE_MULTIPLIER }).map((_, layer) =>
        paths.map((path, idx) => (
          <motion.path
            key={`path-${layer}-${idx}`}
            d={path}
